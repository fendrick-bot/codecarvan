...
class ModelService {
  ...
  async loadModel(customPath?: string): Promise<void> {
    ...
    try {
      ...
      // Load the model using file:// protocol
      // For React Native, we use file:// protocol with the local path
      ...
      // Use file:// protocol for loading
      // If modelPath already has file://, use it directly, otherwise add it
      let fullPath: string;
      if (modelPath.startsWith('file://')) {
        fullPath = `${modelPath}/model.json`;
      } else {
        fullPath = `file://${modelJsonPath}`;
      }
      
      this.model = await tf.loadLayersModel(fullPath);
