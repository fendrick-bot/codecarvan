...
class ModelService {
  ...
  async loadModel(customPath?: string): Promise<void> {
    ...
    try {
      ...
      // Load the model using bundleResourceIO for bundled assets
      // or file:// protocol for file system paths
      console.log(`Loading model from: ${modelPath}`);
      
      // Check if it's a bundled asset (starts with file:// or is a local URI)
      if (modelPath.startsWith('file://') || modelPath.includes('file://')) {
        // File system path
        ...
        // Try using bundleResourceIO for bundled assets
        try {
          const modelJson = require('../assets/models/gemma3/model.json');
